...
test("admin color assignment and public color filtering work end to end", async () => {
  ...
  try {
    ...
    const adminHeaders = {
      Authorization: `Bearer ${signSessionToken(admin)}`,
    ...
    const createColor = await fetch(`${base}/color/create`, {
      method: "POST",
    ...
    assert.equal(
      (
        await fetch(`${base}/color/create`, {
          method: "POST",
          headers: userHeaders,
          body: JSON.stringify({ name: "Denied", hex: "#112233" }),
        })
      ).status,
      403,
    );
    ...
    const edit = await fetch(`${base}/product/edit/${modern._id}`, {
      method: "PUT",
      headers: adminHeaders,
      body: JSON.stringify({ colors: [color._id.toString()] }),
    });
    assert.equal(edit.status, 200);
    assert.deepEqual(
      (await ProductModel.findById(modern._id)).colors.map(String),
      [color._id.toString()],
    );
    ...
    assert.deepEqual(
      new Set(result.data.map((item) => item._id)),
      new Set([modern._id.toString(), legacy._id.toString()]),
    );

    assert.equal(
      (
        await fetch(`${base}/color/status-update/${color._id}`, {
          method: "PATCH",
          headers: adminHeaders,
        })
      ).status,
    ...
    assert.equal(
      (await (await fetch(`${base}/product?color=walnut-brown`)).json()).total,
      0,
    );
  } finally {
