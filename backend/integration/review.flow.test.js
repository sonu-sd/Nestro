...
test("customer review requires approval and updates product rating", async () => {
  ...
  try {
    ...
    assert.equal(
      (await fetch(`${base}/product/slug/missing-chair`)).status,
      404,
    );
    const headers = (person) => ({
      Authorization: `Bearer ${signSessionToken(person)}`,
    ...
    const payload = {
      ...
      comment: "Beautiful finish and comfortable to sit on every day.",
    };
    assert.equal(
      (await (await fetch(`${base}/product?search=oak`)).json()).total,
      1,
    );
    assert.equal(
      (await (await fetch(`${base}/product?search=wood`)).json()).total,
      1,
    );
    assert.equal(
      (await (await fetch(`${base}/product?search=living%20room`)).json())
        .total,
      1,
    );
    assert.equal(
      (await (await fetch(`${base}/product?search=test%20chair`)).json()).total,
      1,
    );
    assert.equal(
      (await (await fetch(`${base}/product?search=%5B`)).json()).total,
      0,
    );
    assert.equal(
      (await (await fetch(`${base}/product?search=unknown`)).json()).total,
      0,
    );
    assert.equal(
      (
    ...
    assert.equal(
      (
        await fetch(`${base}/review`, {
          ...
          body: JSON.stringify({ ...payload, rating: 6 }),
        })
      ).status,
      400,
    );
    const submitted = await fetch(`${base}/review`, {
      method: "POST",
      headers: headers(user),
      body: JSON.stringify(payload),
    });
    ...
    assert.equal(
      (await fetch(`${base}/review/admin`, { headers: headers(user) })).status,
      403,
    );
    assert.equal(
      (
        await (
          await fetch(`${base}/review/admin?status=pending`, {
            headers: headers(admin),
          })
        ).json()
      ).data.length,
    ...
    assert.equal(
      (
        await fetch(`${base}/review/admin/${reviewId}`, {
          method: "PATCH",
          headers: headers(user),
