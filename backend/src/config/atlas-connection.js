...
// Some Windows DNS setups resolve Atlas SRV records through the OS but reject
// Node's c-ares SRV queries. This fallback asks Windows for public DNS records;
// database credentials stay in this process and are never printed or passed to
// PowerShell.
export const getAtlasConnectionUri = async (uri) => {
  const parsed = new URL(uri);
  if (process.platform !== "win32" || parsed.protocol !== "mongodb+srv:")
    return uri;
  ...
  if (!/^[a-z0-9.-]+\.mongodb\.net$/i.test(hostname)) {
    throw new Error(
      "Windows DNS fallback requires an Atlas mongodb.net hostname",
    );
  }
  ...
  const records = Array.isArray(result.srv)
    ? result.srv
    : [result.srv].filter(Boolean);
  if (
    !records.length ||
    records.some(
      ({ host, port }) =>
        !/^[a-z0-9.-]+\.mongodb\.net$/i.test(host) ||
    ...
    throw new Error(
      "Windows DNS fallback returned invalid Atlas server records",
    );
  }

  const options = new URLSearchParams();
  const txtRecords = Array.isArray(result.txt)
    ? result.txt
    : [result.txt].filter(Boolean);
  for (const value of txtRecords) {
    for (const [key, item] of new URLSearchParams(value))
      options.set(key, item);
  }
