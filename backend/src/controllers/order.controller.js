...
const buildOrderNumber = () =>
  `NESTRO-${Date.now()}-${crypto.randomBytes(3).toString("hex").toUpperCase()}`;
...
const canCancel = (order, isAdmin) => {
  const eligibleStatus = isAdmin
    ? ["PENDING", "CONFIRMED", "PROCESSING"]
    : [...CUSTOMER_CANCELLABLE];
  return (
    eligibleStatus.includes(order.orderStatus) && order.paymentStatus !== "PAID"
  );
};

const priceCart = async (userId) => {
  ...
  const products = await ProductModel.find({
    ...
    status: true,
    stock: true,
    category: { $in: activeCategories.map(({ _id }) => _id) },
    roomType: { $in: activeRooms.map(({ _id }) => _id) },
  }).select("title slug price salePrice thumbnail");
  const productsById = new Map(
    products.map((product) => [String(product._id), product]),
  );
  if (cart.items.some((item) => !productsById.has(String(item.productId))))
  ...
  return {
    ...
    tax,
    totalAmount: subtotal + SHIPPING_CHARGE + tax,
  };
};
...
const prepareOrder = async (userId, addressId, paymentMethod) => {
  ...
  const pricedCart = await priceCart(userId);
  const items = pricedCart.items.map(
    ({ originalPrice, slug, ...item }) => item,
  );
  return {
    user: userId,
    ...
    shippingAddress: {
      ...
      address: address.adressLine,
      city: address.city,
      state: address.state,
      pincode: address.pincode,
    },
    subtotal: pricedCart.subtotal,
    shippingCharge: pricedCart.shippingCharge,
    tax: pricedCart.tax,
...
const sendOrderInputError = (res, error) => {
  const messages = {
    ...
    ADDRESS_NOT_FOUND: "Selected delivery address was not found",
    CART_EMPTY: "Your cart is empty",
    PRODUCT_UNAVAILABLE:
      "One or more cart products are unavailable. Please refresh your cart",
  };
  return messages[error.message]
    ? sendBadRequest(res, messages[error.message])
    : null;
};

export const getCheckoutSummary = async (req, res) => {
  try {
    const summary = await priceCart(req.user.id);
    ...
    return sendOrderInputError(res, error) || sendServerError(res, error);
  }
};
  ...
  try {
    if (req.body.paymentMethod !== "COD")
      return sendBadRequest(
        res,
        "Use the online payment endpoint for online payments",
      );
...
export const createOnlineOrder = async (req, res) => {
  try {
    const payload = await prepareOrder(
      req.user.id,
      req.body.addressId,
      "ONLINE",
    );
    ...
    try {
      const razorpayOrder = await createRazorpayOrder({
        ...
        notes: { nestroOrderId: String(order._id) },
      });
      ...
      if (paymentError.message === "RAZORPAY_NOT_CONFIGURED")
        return sendServerError(res, paymentError);
      throw paymentError;
    }
  } catch (error) {
    return sendOrderInputError(res, error) || sendServerError(res, error);
...
export const verifyOnlinePayment = async (req, res) => {
  try {
    ...
    if (
      ...
      return sendBadRequest(res, "Invalid payment verification data");
    ...
    if (!signatureMatches)
      return sendBadRequest(res, "Payment signature verification failed");
    ...
    if (payment.status !== "captured")
      return res.status(409).json({
        success: false,
        message:
          "Payment is awaiting capture. Please check your orders shortly.",
      });
    ...
    if (order.paymentStatus !== "PAID") {
      ...
      await cartModel.updateOne(
        { userId: order.user },
        {
          $pull: {
            items: {
              productId: { $in: order.items.map((item) => item.product_id) },
            },
          },
        },
      );
...
export const getAdminOrders = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      limit,
      pages: Math.ceil(total / limit),
      summary: { all, pending, revenue: revenue[0]?.total || 0 },
    });
  } catch (error) {
    return sendServerError(res, error);
  }
...
export const updateAdminOrder = async (req, res) => {
  try {
    if (!isValidId(req.params.id))
      return sendBadRequest(res, "Invalid order id");
    const { orderStatus, trackingNumber, courierName, cancellationReason } =
      req.body;
    const order = await OrderModel.findById(req.params.id);
    if (!order) return sendNotFound(res, "Order not found");
    if (!ADMIN_TRANSITIONS[order.orderStatus]?.includes(orderStatus))
      return sendBadRequest(res, "Invalid order status transition");
    if (orderStatus === "CANCELLED") {
      if (!canCancel(order, true))
        return sendBadRequest(
          res,
          order.paymentStatus === "PAID"
            ? "Paid orders require a refund before cancellation"
    ...
    if (orderStatus === "SHIPPED") {
      if (
        ...
        return sendBadRequest(
          res,
          "Tracking number and courier name are required before shipping",
        );
