...
const findCategory = async (id, activeOnly = false) => {
    ...
    return categoryModel.findOne(activeOnly ? { _id: id, status: true } : { _id: id });
};

export const readById = async (req, res) => {
    try {
        if (!isValidId(req.params.id)) return sendBadRequest(res, "Invalid category id");
...
export const create = async (req, res) => {
    try {
        ...
        if (await categoryModel.exists({ slug })) return sendConflict(res, "A category already uses this slug");
        const image = getUploadMetadata(req.file);
        const category = await categoryModel.create({ name: req.body.name.trim(), slug, image: image.url, imagePublicId: image.publicId });
...
export const updateStatus = async (req, res) => {
    try {
        ...
        await category.save();
        return sendSuccess(res, category.status ? "Category restored" : "Category archived");
    } catch (error) { return sendServerError(res, error); }
};
...
export const edit = async (req, res) => {
    try {
        ...
        if (slug && slug !== category.slug && await categoryModel.exists({ slug })) return sendConflict(res, "A category already uses this slug");
        if (name) category.name = name;
        if (slug) category.slug = slug;
        const image = getUploadMetadata(req.file);
        const oldPublicId = image.url ? category.imagePublicId : "";
        if (image.url) { category.image = image.url; category.imagePublicId = image.publicId; }
        await category.save();
...
export const deleteById = async (req, res) => {
    try {
        ...
        const category = await findCategory(req.params.id);
        if (!category) return sendNotFound(res, "Category not found");
        if (!category.status) return sendSuccess(res, "Category is already archived");
