...
const pageSize = (value, fallback) =>
  Math.max(1, Math.min(30, Number.parseInt(value, 10) || fallback));
  ...
  try {
    ...
    const [data, total] = await Promise.all([
      Review.find(filter)
        ...
        .limit(limit)
        .populate("user", "name")
        .populate("product", "title slug status thumbnail"),
      Review.countDocuments(filter),
    ]);
    return res.json({
      success: true,
      data: data.filter((review) => review.user && review.product?.status),
      total,
      page,
      limit,
    });
...
export async function createReview(req, res) {
  try {
    ...
    if (
      ...
      return sendBadRequest(res, "Title must be 3–80 characters");
    if (
      typeof comment !== "string" ||
      comment.trim().length < 20 ||
      comment.trim().length > 1000
    )
      return sendBadRequest(res, "Review must be 20–1000 characters");
    if (!(await Product.exists({ _id: product, status: true })))
      return sendNotFound(res, "Product not found");
    if (await Review.exists({ user: req.user._id, product }))
      return sendConflict(res, "You have already reviewed this product");
    ...
    return res
      ...
      .json({
        ...
        data: { _id: review._id, status: review.status },
      });
  } catch (error) {
    if (error.code === 11000)
      return sendConflict(res, "You have already reviewed this product");
...
export async function myReviews(req, res) {
  try {
    const data = await Review.find({ user: req.user._id })
      .sort({ createdAt: -1 })
...
export async function moderateReview(req, res) {
  try {
    if (!validId(req.params.id))
      return sendBadRequest(res, "Invalid review id");
    if (!["approved", "rejected", "pending"].includes(req.body?.status))
      return sendBadRequest(res, "Invalid review status");
