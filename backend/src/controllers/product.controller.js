...
const POPULATE = [
  ...
  { path: "roomType", select: "name slug status" },
...
const validateColorReferences = async (colors, existing = []) => {
  if (
    ...
    colors.length > 12 ||
    colors.some((id) => !isValidId(id))
  )
    return "Please select valid colors";
  if (!colors.length) return null;
  const count = await colorModel.countDocuments({
    _id: { $in: colors },
    $or: [{ status: true }, { _id: { $in: existing } }],
  });
...
const getActiveReferences = async () => {
  ...
  return {
    ...
    rooms: rooms.map(({ _id }) => _id),
  };
};

const validateReferences = async (category, roomType) => {
  if (!isValidId(category) || !isValidId(roomType))
    return "Please provide valid category and room type";
  ...
  return activeCategory && activeRoom
    ? null
    : "Category and room type must be active";
};
...
const parseProductValues = (body) => {
  ...
  ["length", "width", "height"].forEach((key) => {
    if (values[key] !== undefined) {
      dimensions[key] = Number(values[key]);
      delete values[key];
    }
  });
  if (Object.keys(dimensions).length) values.dimensions = dimensions;
  if (values.weight !== undefined) {
    values.weight = { value: Number(values.weight) };
  }
  return values;
};
...
const validateProductValues = (values, isCreate = false) => {
  ...
  if (
    ...
    return "Required product fields are missing";
  if (
    values.title !== undefined &&
    (typeof values.title !== "string" || values.title.trim().length < 2)
  )
    return "Title must be at least 2 characters";
  if (values.slug !== undefined && !values.slug)
    return "Please provide a valid slug";
  if (values.material !== undefined && !MATERIALS.has(values.material))
    return "Invalid material";
  if (
    values.price !== undefined &&
    (!Number.isFinite(values.price) || values.price < 200)
  )
    return "Price must be at least 200";
  if (
    values.salePrice !== undefined &&
    (!Number.isFinite(values.salePrice) || values.salePrice < 0)
  )
    return "Sale price must be zero or more";
  if (
    values.discount !== undefined &&
    (!Number.isFinite(values.discount) ||
      values.discount < 0 ||
      values.discount > 100)
  )
    return "Discount must be between 0 and 100";
  if (
    ["stock", "featured", "bestSeller", "newArrival", "status"].some(
      (key) => Object.hasOwn(values, key) && values[key] === undefined,
    )
  )
    return "Invalid product setting";
  if (
    values.dimensions &&
    Object.values(values.dimensions).some(
      (value) => !Number.isFinite(value) || value < 0,
    )
  )
    return "Dimensions must be zero or more";
  if (
    values.weight &&
    (!Number.isFinite(values.weight.value) || values.weight.value < 0)
  )
...
const buildPublicFilter = async (query) => {
  ...
  Object.entries(booleanFilters).forEach(([queryKey, field]) => {
    if (query[queryKey] !== undefined)
      filter[field] = parseBoolean(query[queryKey]);
  });
  if (query.category) {
    const slugs = query.category.split(",").map(normalizeSlug).filter(Boolean);
    const matches = await categoryModel
      .find({ slug: { $in: slugs }, status: true })
      .select("_id");
    filter.category = { $in: matches.map(({ _id }) => _id) };
  }
  if (query.room) {
    const slugs = query.room.split(",").map(normalizeSlug).filter(Boolean);
    const matches = await roomModel
      .find({ slug: { $in: slugs }, status: true })
      .select("_id");
    filter.roomType = { $in: matches.map(({ _id }) => _id) };
  }
  if (query.color) {
    const slugs = String(query.color)
      .split(",")
      .map(normalizeSlug)
      .filter(Boolean);
    const matches = await colorModel
      .find({ slug: { $in: slugs }, status: true })
      .select("_id name");
    const legacyNames = matches.map(
      ({ name }) =>
  ...
  if (query.minprice !== undefined || query.maxprice !== undefined) {
    if (
      (query.minprice !== undefined && !Number.isFinite(minimum)) ||
      (query.maxprice !== undefined && !Number.isFinite(maximum)) ||
  ...
  if (query.material)
    filter.material = {
      $in: query.material.split(",").filter((value) => MATERIALS.has(value)),
    };
  if (query.search) {
    const search = String(query.search)
      ...
      .replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    if (search) {
      const [matchingCategories, matchingRooms, matchingColors] =
        await Promise.all([
          categoryModel
            .find({
              status: true,
      ...
      const matches = {
        $or: [
          ...
          { roomType: { $in: matchingRooms.map(({ _id }) => _id) } },
          { colors: { $in: matchingColors.map(({ _id }) => _id) } },
        ],
      };
      filter.$and = [...(filter.$and || []), matches];
    }
  }
  return filter;
};
...
const sortFor = (sort) =>
  ({
    ...
    low: { salePrice: 1 },
    high: { salePrice: -1 },
    bestselling: { bestSeller: -1, createdAt: -1 },
    newest: { createdAt: -1 },
  })[sort] || { createdAt: -1 };
...
export const read = async (req, res) => {
  try {
    ...
    const limit = parseBoundedNumber(req.query.limit, 9, 1, 100);
    ...
    if (error.message === "INVALID_PRICE_FILTER")
      return sendBadRequest(res, "Invalid price filter");
...
export const readAdmin = async (req, res) => {
  try {
    ...
    const [data, total, all, active, outOfStock] = await Promise.all([
      ...
      ProductModel.countDocuments({ stock: false }),
    ]);
    return res
      .status(200)
      .json({
        ...
        limit,
        pages: Math.ceil(total / limit),
        summary: { all, active, archived: all - active, outOfStock },
      });
  } catch (error) {
    return sendServerError(res, error);
  }
...
export const create = async (req, res) => {
  try {
    ...
    if (values.salePrice > values.price)
      return sendBadRequest(res, "Sale price cannot exceed regular price");
    if (!req.file)
      return sendBadRequest(res, "A product thumbnail is required");
    ...
    if (values.colors !== undefined) {
      const colorError = await validateColorReferences(values.colors);
      if (colorError) return sendBadRequest(res, colorError);
    }
    if (await ProductModel.exists({ slug: values.slug }))
      return sendConflict(res, "A product already uses this slug");
...
export const edit = async (req, res) => {
  try {
    ...
    if (validationError) return sendBadRequest(res, validationError);
    if (
      (values.salePrice ?? product.salePrice) > (values.price ?? product.price)
    )
      return sendBadRequest(res, "Sale price cannot exceed regular price");
    if (
      values.slug &&
      values.slug !== product.slug &&
      (await ProductModel.exists({ slug: values.slug }))
    )
      return sendConflict(res, "A product already uses this slug");
    ...
    if (referenceError) return sendBadRequest(res, referenceError);
    if (values.colors !== undefined) {
      const colorError = await validateColorReferences(
        values.colors,
        product.colors,
      );
      if (colorError) return sendBadRequest(res, colorError);
    ...
    if (values.dimensions) {
      product.dimensions = { ...product.dimensions, ...values.dimensions };
      delete values.dimensions;
    }
    if (values.weight) {
      product.weight = { ...product.weight, ...values.weight };
    ...
    if (thumbnail.url) {
      ...
      product.thumbnailPublicId = thumbnail.publicId;
    }
    await product.save();
...
export const addImages = async (req, res) => {
  try {
    ...
    if (!product) {
      await removeCloudinaryAssets(req.files?.map((file) => file.filename));
      return sendNotFound(res, "Product not found");
    }
    const uploads = req.files?.map(getUploadMetadata) || [];
    if (!uploads.length)
      return sendBadRequest(res, "Please choose at least one image");
    if (product.images.length + uploads.length > 6) {
      await removeCloudinaryAssets(uploads.map(({ publicId }) => publicId));
      return sendBadRequest(res, "A product can have at most 6 images");
    }
...
export const updateStatus = async (req, res) => {
  try {
    ...
    return sendSuccess(
      res,
      product.status ? "Product restored" : "Product archived",
    );
  } catch (error) {
    return sendServerError(res, error);
  }
...
export const updateFlag = async (req, res) => {
  try {
    ...
    if (!allowedFields.includes(req.body.field))
      return sendBadRequest(res, "Invalid product field");
    const product = await ProductModel.findById(req.params.id);
    if (!product) return sendNotFound(res, "Product not found");
