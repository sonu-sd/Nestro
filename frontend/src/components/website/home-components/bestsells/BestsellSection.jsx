...
export default async function BestsellSection({ products }) {
  return (
    <section className="px-4 py-8 sm:px-6 lg:px-8">
      ...
      <h2 className="mt-2 text-xl font-medium text-[#1e1e1e]">Best Sellers</h2>

      <div className="mt-5 grid grid-cols-2 gap-2.5 sm:gap-4 lg:grid-cols-4 lg:gap-6">
        {products?.length ? (
          products.map((item) => (
      ...
      <Link
        href="/store?sort=bestselling"
        className="mt-4 inline-block text-sm font-semibold text-[#8B5E3C] hover:underline"
      >
