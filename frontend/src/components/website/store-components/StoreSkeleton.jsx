export default function StoreSkeleton({ sidebar = false }) {
  ...
  if (sidebar)
    return (
      <div
        aria-busy="true"
        className="rounded-2xl border border-[#E8DDD3] bg-white p-6"
      >
        ...
        {Array.from({ length: 4 }, (_, index) => (
          <div key={index} className="mt-7 space-y-3">
            ...
            {Array.from({ length: 3 }, (_, row) => (
              <div key={row} className={`${bar} h-3 w-full`} />
            ))}
          </div>
        ))}
      </div>
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-4 xl:grid-cols-3">
        {Array.from({ length: 6 }, (_, index) => (
          <div
            key={index}
            className="overflow-hidden rounded-xl border border-[#E8DDD3] bg-white"
          >
            <div className={`${bar} aspect-square w-full rounded-none`} />
