...
function VerifiedAdminGate({ children }) {
  ...
  useEffect(() => {
    ...
    client
      ...
      .then(({ data }) => {
        ...
        setState({
          loading: false,
          user: ["admin", "superAdmin"].includes(user?.role) ? user : null,
          error:
          ...
          setState({
            ...
            error:
              error.response?.status === 401
                ...
                : "Unable to verify your admin session. Please try again.",
          });
      });
    return () => {
      active = false;
  ...
  return (
    <AdminContext.Provider value={state.user}>{children}</AdminContext.Provider>
  );
}
