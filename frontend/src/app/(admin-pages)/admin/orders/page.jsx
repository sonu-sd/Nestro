...
export default function AdminOrdersPage() {
  ...
  const loadOrders = async () => {
    try {
      const response = await client.get("/order/admin");
  ...
  const updateOrder = async (order, status) => {
    ...
    if (status === "CANCELLED")
      payload.cancellationReason =
        window.prompt("Cancellation reason:") || "Cancelled by admin";
    try {
      await client.patch(`/order/admin/${order._id}`, payload);
      await loadOrders();
    } catch (requestError) {
      setError(
    ...
    <main className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Manage fulfilment and delivery status.
        </p>
      </div>
      {error && (
            ...
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <tr key={order._id} className="border-t border-gray-100">
                  ...
                  <td className="p-4">
                    {NEXT_STATUS[order.orderStatus]?.length ? (
                      <select
                        defaultValue=""
                        onChange={(event) => {
                          if (event.target.value)
                            void updateOrder(order, event.target.value);
                        }}
