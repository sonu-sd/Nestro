...
export default function EditProductPage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([client.get(`/product/admin/${id}`), client.get("/category"), client.get("/room-type"), client.get("/color/admin")])
      .then(([productResponse, categoryResponse, roomResponse, colorResponse]) => {
        if (!active) return;
        const item = productResponse.data.data;
        setForm({ title: item.title || "", slug: item.slug || "", shortDescription: item.shortDescription || "", description: item.description || "", category: item.category?._id || item.category || "", roomType: item.roomType?._id || item.roomType || "", price: item.price ?? "", salePrice: item.salePrice ?? "", discount: item.discount ?? "", material: item.material || "Wood", color: item.color || "", colors: (item.colors || []).map((color) => color?._id || color), length: item.dimensions?.length ?? "", width: item.dimensions?.width ?? "", height: item.dimensions?.height ?? "", weight: item.weight?.value ?? "", stock: item.stock, featured: item.featured, bestSeller: item.bestSeller, newArrival: item.newArrival, status: item.status });
        setPreview(item.thumbnail || ""); setCategories(categoryResponse.data.data || []); setRooms(roomResponse.data.data || []); setAvailableColors(colorResponse.data.data || []);
  ...
  async function submit(event) {
    ...
    Object.entries(form).forEach(([key, value]) => { if (value !== "") body.append(key, key === "colors" ? value.join(",") : value); });
  ...
  if (!form) return <AdminSkeleton variant="form"/>;
