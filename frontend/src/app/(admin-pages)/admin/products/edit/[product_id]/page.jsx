...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      client.get("/color/admin"),
    ])
      .then(
        ([productResponse, categoryResponse, roomResponse, colorResponse]) => {
          if (!active) return;
          ...
          setForm({
            ...
            roomType: item.roomType?._id || item.roomType || "",
            price: item.price ?? "",
            salePrice: item.salePrice ?? "",
            discount: item.discount ?? "",
            material: item.material || "Wood",
            color: item.color || "",
            colors: (item.colors || []).map((color) => color?._id || color),
          ...
          setAvailableColors(colorResponse.data.data || []);
        },
      )
      .catch((requestError) => {
        if (active)
    ...
    setForm((current) => {
      ...
      if (name === "price" || name === "salePrice") {
        const price = Number(next.price),
          sale = Number(next.salePrice);
        next.discount =
          price > 0 && sale >= 0
            ? Math.max(0, Math.round((1 - sale / price) * 100))
            : "";
      }
      return next;
    });
  ...
  async function submit(event) {
    ...
    Object.entries(form).forEach(([key, value]) => {
      if (value !== "")
        body.append(key, key === "colors" ? value.join(",") : value);
    });
    if (thumbnail) body.append("image", thumbnail);
    setSaving(true);
    try {
      await client.put(`/product/edit/${id}`, body);
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Update catalog information and merchandising settings.
          </p>
        </div>
        <form onSubmit={submit} className="space-y-6">
          ...
          <Section title="Availability and merchandising">
            <div className="grid gap-3 sm:grid-cols-2">
              {flags.map(([name, label]) => (
                <label
                  key={name}
                  className="flex items-center justify-between rounded-lg border border-slate-200 p-3 text-sm font-medium"
                >
                  {label}
                  <input
                    type="checkbox"
          ...
          <div className="flex gap-3">
            <button
              disabled={saving}
              className="rounded-lg bg-teal-700 px-6 py-3 text-sm font-semibold text-white disabled:opacity-50"
            >
...
function Field({ label, multiline, ...props }) {
  const className =
    "mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 text-sm outline-none focus:border-teal-600";
  return (
    <label className="block text-sm font-medium">
      {label}
      {multiline ? (
        <textarea rows={4} className={className} {...props} />
      ) : (
