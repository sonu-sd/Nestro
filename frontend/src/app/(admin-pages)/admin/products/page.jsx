...
export default function ProductsPage() {
  ...
  async function load() {
    try {
      setError("");
      const { data } = await client.get("/product/admin", {
        params: { page, search: query, status: filter },
      });
      setProducts(data.data || []);
      setPages(data.pages || 0);
      setTotal(data.total || 0);
      setSummary(data.summary || { all: 0, active: 0, archived: 0 });
    } catch (requestError) {
      setError(
  ...
  async function toggleStatus(product) {
    if (
      !window.confirm(
        `${product.status ? "Archive" : "Restore"} ${product.title}?`,
      )
    )
      return;
    try {
    ...
    <main className="min-h-screen bg-slate-50 p-4 text-slate-900 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Review, edit and publish your store catalog.
            </p>
          </div>
          <Link
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Stat label="Total products" value={summary.all} />
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap gap-3 border-b border-slate-100 p-4">
            <input
              ...
              className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-teal-600"
            />
            <select
          ...
          <div className="flex items-center justify-between border-t p-4 text-sm text-slate-500">
            <span>
              {total} matching products · Page {page} of {Math.max(pages, 1)}
            </span>
